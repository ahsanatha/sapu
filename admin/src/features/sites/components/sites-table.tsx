...
export function SitesTable({ data, search, navigate }: DataTableProps) {
  ...
  const { columnFilters, onColumnFiltersChange, pagination, onPaginationChange, ensurePageInRange } = useTableUrlState({
    ...
    globalFilter: { enabled: true, key: 'name' },
    columnFilters: [
  ...
  const enabledOptions = useMemo(() => [{ label: 'true', value: 'true' }, { label: 'false', value: 'false' }], [])

  return (
    <div className={cn('max-sm:has-[div[role="toolbar"]]:mb-16', 'flex flex-1 flex-col gap-4')}>
      <DataTableToolbar table={table} searchPlaceholder='Filter sites...' searchKey='name' filters={[{ columnId: 'enabled', title: 'Enabled', options: enabledOptions }]} />
